...
export interface MultimodalElementItem {
    alttext: string;
    caption?: string;
    height: number;
    id: string;
...
/**
 * A reference to a file that has already been uploaded by the host solution.
 * No file content is transferred through this API - the AI Assistant resolves
 * the file using the artifactId.
 */
export interface AIAttachment {
    /**
     * The id of the previously-uploaded artifact.
     */
    artifactId: string;
    /**
...
export interface SuccessResponse {
    ...
    confidence?: {
        ...
        confidenceIndicators?: {
            ...
            insufficientContentDetected?: boolean;
        };
    };
...
export declare enum AIMessageType {
    ...
    QUERY = "QUERY",
    RESPONSE = "RESPONSE",
    SET_FORM_FACTOR = "SET_FORM_FACTOR",
...
export interface AIMessagePayload {
    /**
     * References to files that have already been uploaded by the host solution,
     * to include as context for the message.
     */
...
export declare enum AIApplicationId {
    ...
    EXPERIMENTATIONACCELERATOR = "experimentation_agent",
    GENSTUDIO = "genstudio",
    IVO = "ivo-ui",
...
export interface AppContext {
    /**
     * Whether the AI/Coworker rail is enabled for the current app and context, i.e.
     * whether `ai.openChat` will actually surface a panel. Mirrors the shell's own
     * rail-render gate (`isAIRailEnabled && !disabledTab`). May be `undefined` when
     * served by an older shell that does not yet report it.
     */
    isAIRailEnabled?: boolean;
    /**
     * Whether Coworker is enabled for the current org/context (the `shell-coworker-enabled`
     * or `coworker-incubator-enabled` flag). Lets a host app choose the correct fallback
     * destination when no rail is available. May be `undefined` on an older shell.
     */
    isCoworkerEnabled?: boolean;
...
export interface OpenChatProps {
    /**
     * Custom app context that host app can pass in when the chat is open.
     * This is an alternative way to pass in app context in comparison to Interframe's APP_CONTEXT request.
     */
    appContext?: JSONValue;
    /**
     * References to files that the host solution has already uploaded, to pre-fill
     * as context for the first message.
     */
    attachments?: AIAttachment[];
    /**
...
export interface AIApi extends EventEmitter<AIMessageEvent> {
    ...
    closeChat: () => void;
    /**
     * Requests the documentation sources object from the configuration object.
     *
     * ***Example:***
     *
     * ```typescript
     * ai.getDocumentationSources();
     * ```
     */
    getDocumentationSources: () => Promise<Partial<Record<AIApplicationId, string>>>;
    /**
     ...
     * ai.openChat({ appContext: { data: { ... } } });
     * ai.openChat({ attachments: [{ artifactId: 'art-123', contentType: 'application/pdf', name: 'report.pdf' }], query: 'Summarize this file' });
     * ```
     */
    ...
    /**
     * Adds a button to the not provisioned dialog that calls a callback function when pressed.
     * Designed to be used by Workfront to launch their review agreement dialog.
     * @param callback Function to be called on button click
     */
    setProvisioningButton: (callback: () => void) => void;
    /**
    ...
    /**
     * Sends a message to Unified Shell to toggle the AI panel expansion.
     *
     * ***Example to expand the panel:***
     *
     * ```typescript
     * ai.toggleExpansion(true);
     * ```
     */
