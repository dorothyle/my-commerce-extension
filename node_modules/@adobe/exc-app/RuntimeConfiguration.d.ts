...
import type { Sandbox, SubOrg } from './appcontext';
import type { ShellInfoObject } from './shell';
export type { SubOrg };
...
export interface RuntimeConfiguration extends BaseNetworkConfig {
     ...
     */
    aiDocumentationSources: Record<AIApplicationId, string>;
    /**
    ...
    /**
     * If headerless is true, the shell header is hidden.
     */
    headerlessMode?: boolean;
    /**
     ...
     * rail-render gate (`isAIRailEnabled && !disabledTab`).
     */
    ...
    isAWSOrg?: boolean;
    /**
     * Whether Coworker is enabled for the current org/context (the
     * `shell-coworker-enabled` or `coworker-incubator-enabled` flag). Lets an app
     * choose the correct fallback destination when no rail is available.
     */
    ...
    /**
     * Is the right panel expanded (full width)
     */
     ...
     * within the application. Be aware that this value will be updated on every navigation
     * including any redirections before the page has fully loaded so it is best to wait until
     * the page loading is complete before using this value.
     */
    pageUrl?: string;
    /**
    ...
    sandbox?: Sandbox;
    /**
     * Height of the shell header only - does not include banners (announcement,
     * network disconnect, etc). See `totalHeaderHeight` for the banner-inclusive total.
     */
    shellHeight: number;
    /**
     * Shell Info
    ...
    /**
     * Total height of the shell chrome above the app: the header plus any banners
     * (announcement, network disconnect, etc). Equals `shellHeight` + banner heights.
     */
    ...
    /**
     * List of current user orgs instances
     */
